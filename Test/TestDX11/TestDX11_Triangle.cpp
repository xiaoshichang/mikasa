...
void InitPipeline() {
    ...
    result = D3DCompileFromFile(L"sample.vs", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "vs_5_0", dwShaderFlags, 0, &VS, &errorMessage);
    if (FAILED(result))
    {
    ...
    result = D3DCompileFromFile(L"sample.ps", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "main", "ps_5_0", dwShaderFlags, 0, &PS, &errorMessage);
    if (FAILED(result))
    {
        OutputDebugStringA(reinterpret_cast<const char*>(errorMessage->GetBufferPointer()));
        std::cout << reinterpret_cast<const char*>(errorMessage->GetBufferPointer()) << std::endl;
    ...
    D3D11_INPUT_ELEMENT_DESC ied[] =
            {
                    ...
                    {"ATTR", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0},
            };
